...
function resize(bigImg: HTMLElement): void {
  const width = bigImg.getBoundingClientRect().width;
  const height = bigImg.getBoundingClientRect().height;
  if ( screen.width <= mobileSize ){
    if ( width > height ){
      bigImg.setAttribute('style',
      'width: 100vw; height: unset; maxWidth: unset; margin-top: unset; margin: 0; bottom: 30%;');
    } else if ( width < height ){
      bigImg.setAttribute('style',
      'width: unset; height: 100vh; maxWidth: unset; margin-top: unset; margin: 0; bottom: 15%;');
    }
  }
}
function switchResize(): void{
  if ( screen.width <= mobileSize ){
    $('.modal .imgs .img.active').each((i, el) => {
      const width = el.getBoundingClientRect().width;
      const height = el.getBoundingClientRect().height;
      if ( width < height ){
        el.setAttribute('style', 'bottom: 15%;');
      } else {
        el.setAttribute('style', 'bottom: 30%;');
      }
    });
  }
}
...
export class MockupComponent implements AfterViewInit, OnInit {
  ...
  openModal(id: number): void{
    ...
    if ( screen.width <= mobileSize ){
      $('#img_1_7, #thumb_1_7').css('display', 'none');
    }
    const bigPic = $('.modal:nth-child(' + id + ') > .imgs > .img.active');
    const width = Number(bigPic.width());
    const height = Number(bigPic.height());
    if ( screen.width <= mobileSize && width < height) {
      bigPic.css('bottom', '10%');
    } else {
      bigPic.css('bottom', '33%');
    }
  }
